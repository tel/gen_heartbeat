...
-export_type([option/0, command/0]).

-behaviour(gen_server).
-export([init/1, terminate/1, handle_call/3, 
...
-spec 
start(Module :: module(), Args :: list(option())) -> {ok,Pid} | ignore | {error,Error}.
start(Module, Args) -> 
...
-spec 
start_link(Module :: module(), Args :: list(option())) -> {ok,Pid} | ignore | {error,Error}.
...
stop(Server) ->
    gen_server:call(Server, {stop, user}).
...
insepct(Server) -> gen_server:call(Server, inspect).
...
-callback termiante(Reason :: term(), State :: _) -> 'ok'.
...
terminate(Reason, State = #st{tref = TRef, module = Module, state = MState}) -> 
...
handle_info({update_state, NewMState}, State) -> {noreply, State#{state = NewMState}};
handle_info(beat, 
    ...
    {MsTime, NewState} = 
        case timer:tc(Module, beat, MState0) of
            {RTime*1000, ok} -> 
                {RTime, State};
            {RTime*1000, MState1} ->
                {RTime, State#st{state = MState1}}
        end,
    lager:info("~s: heatbeat in ~bms", [Name, MsTime]),
    zeta:cvh(["heartbeat", Name, "beat_time"], MsTime),
    {ok, NewState}.
